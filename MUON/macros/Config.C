...
void Config(const char *directory = "", const char *option = "param",
            const char *digitstore = "AliMUONDigitStoreV2S",
            bool forEmbedding = kFALSE, bool forRealistic = kFALSE) {
  //=====================================================================
  ...
  AliRunLoader *rl = 0x0;
  rl = AliRunLoader::Open(filename, AliConfig::GetDefaultEventFolderName(),
                          "recreate");
  if (rl == 0x0) {
  ...
  if (!strcmp(option, "param")) {
    ...
    gener->SetOrigin(0, 0, 0); // vertex position    gener->SetSigma(0,0,0);
  ...
  // activate trigger cluster-size (0=default, 1=cluster-size according to
  ...
  // activate trigger chamber efficiency by cells (0=default, 1=trigger
  // efficiency according to AliMUONTriggerEfficiencyCells
  //  MUON->SetTriggerEffCells(0);

  // Activate the following line when running with Fluka
  // MUON->SetIsMaxStep(kFALSE);

  // Use SetDigitStoreClassName() to change the digitStore implementation used
  // by (s)digitizer
  MUON->SetDigitStoreClassName(digitstore);

  if (forEmbedding) {
    // Noise-only digits in tracker/trigger (0=no noise, 1=default (noise in
    // tracker), 2=noise in tracker and trigger):
    cout << "****** DISABLING NOISE GENERATION AS WE DO EMBEDDING ******"
         << endl;
